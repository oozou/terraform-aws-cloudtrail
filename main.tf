...
resource "aws_iam_role" "cloudtrail_put_log_cw" {
  ...
  managed_policy_arns = [aws_iam_policy.cloudtrail_put_log_cw.arn]
...
resource "aws_cloudwatch_log_group" "trail_log" {
  ...
  tags = merge(local.tags, { Name = "${local.name}-log-group" })
}
...
resource "aws_cloudtrail" "this" {
  ...
  s3_bucket_name                = var.centralize_trail_logging_bucket_name
  include_global_service_events = var.include_global_service_events
  cloud_watch_logs_group_arn    = "${aws_cloudwatch_log_group.trail_log.arn}:*"
  cloud_watch_logs_role_arn     = aws_iam_role.cloudtrail_put_log_cw.arn
